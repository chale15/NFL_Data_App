...
def top_names_state_plot(df, year=2000, n=10, width=800, height=600, variable='count', state = ''):
    color_map = {"M": "#636EFA", "F": "#EF553B"}
    year_data = df[df['year'] == year].copy()
    year_data['overall_rank'] = year_data[variable].rank(method='min', ascending=False).astype(int)
...
def get_random_year(df):
    years = df['year']
    random_year = years[random.randint(0, len(years))]
    return random_year



def name_trend_line(df, name = ''):
    ...
    fig = px.line(
        ...
        y = 'count', 
        color='sex',
        title=f"Popularity of the name {name}",
        color_discrete_map=color_map,
        labels={"sex": "Sex"}
    )
    ...
    fig.update_layout(
        xaxis_title="Year",
        yaxis_title=f"Babies Born Named {name}",
    )
    
    return fig

def names_trend_line(df, names = ['']):
    name_df = df[df['name'].isin(names)]
    name_df = name_df.groupby(['year', 'name'], as_index=False)['count'].sum()
    color_map = {name: f'#{hash(name) & 0xFFFFFF:06x}' for name in names}
    ...
    fig = px.line(
        ...
        y='count', 
        color='name',
        title=f"Popularity of Names: {' vs. '.join(names)}",
        color_discrete_map=color_map,
        labels={"name": "Name", "count": "Babies Born"}
    )
...
def name_trend_stats(df, name = ''):
    
    name_df = df[(df['name']==name)]
    first_year = name_df['year'].min()
    max_count = name_df['count'].max()
    year_max = name_df[name_df['count'] == max_count]['year'].iloc[0]
